...
##	* public domain *
##
##	removed abstfilter dependencies
...
class SExprReader():
	...
	def feed(self, tokens):
		for (i,c) in enumerate(tokens):
			if self.incomment:
			...
			elif self.inescape or ((c not in self.special) and (not self.inquote)):
				...
				if self.sym:
					...
					if self.build == None:
						self.next_filter.feed(self.sym)
					else:
				...
				if c in self.comment_begin:
					...
					if self.build == None:
						...
						if len(self.build_stack) == 1:
							...
							self.next_filter.feed(self.build)
